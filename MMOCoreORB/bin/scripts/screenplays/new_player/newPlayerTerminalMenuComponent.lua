...
newPlayerTerminalMenuComponent = {  }

function newPlayerTerminalMenuComponent:fillObjectMenuResponse(pSceneObject, pMenuResponse, pPlayer)
	local response = LuaObjectMenuResponse(pMenuResponse)
	response:addRadialMenuItem(120, 3, "Get Buffs")
	response:addRadialMenuItem(121, 3, "Clear Wounds")
	
end

function newPlayerTerminalMenuComponent:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	if selectedID == 120 then
	  ...
	  local sui = SuiMessageBox.new("newPlayerTerminalMenuComponent", "handleBuffChoice")
	...
	elseif selectedID == 121 then
	  ...
	  local sui = SuiMessageBox.new("newPlayerTerminalMenuComponent", "handleWoundChoice")
...
function newPlayerTerminalMenuComponent:handleBuffChoice(pPlayer, pSui, eventIndex, ...)
  if (pPlayer == nil) then
    return
  end
  ...
  elseif (eventIndex == 0) then -- Buff the player
    CreatureObject(pPlayer):enhanceCharacterDroid()
...
function newPlayerTerminalMenuComponent:handleWoundChoice(pPlayer, pSui, eventIndex, ...)
