...
StructuresVendorLogic = ScreenPlay:new {
	scriptName = "StructuresVendorLogic",
	currencies = {
			{currency = "credits"},
			{currency = "experience", name = "Starforge Currency", experience =  "starforge_currency"},
		        },

		merchandise_structures = { -- Displayed name, full template string (without the shared_), cost {} - follow same order as the currencies setup previously
		{name = "Schematic: Heavy Automated Creature Processing Plant", template = "object/tangible/loot/loot_schematic/structure/harvester_creature_heavy_schematic.iff", cost = {0, 50000}},
	    },
}
...
function StructuresVendorLogic:openSUIStructures(pCreatureObject, pUsingObject)
	...
	sui.setTitle("Structure Schematics")

	local message = "Please select which item you want to buy."
	sui.setPrompt(message)

	for i = 1, #self.merchandise_structures, 1 do
		local merchString = self:getMerchandiseStringStructures(i)
		sui.add(merchString, "")
	end
...
function StructuresVendorLogic:buyItemStructures(pPlayer, itemSelected)
	local merch = self.merchandise_structures[itemSelected]
	...
	local canPurchase = self:hasEnoughCurrencyStructures(pPlayer, itemSelected)
		...
		for i = 1, #merch.cost do
			...
			if (cost ~= 0) then
				if (currency == "token") then
					self:payTokensStructures(pPlayer, itemSelected, i)
				end
...
function StructuresVendorLogic:getMerchandiseStringStructures(num)
	local merch = self.merchandise_structures[num]
...
function StructuresVendorLogic:hasEnoughCurrencyStructures(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function StructuresVendorLogic:payTokensStructures(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
	
	local merch = self.merchandise_structures[selectedItem]
