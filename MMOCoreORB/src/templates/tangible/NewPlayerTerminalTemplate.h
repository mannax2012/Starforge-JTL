/*
 * NewPlayerTerminalTemplate.h
 *
 *  Created on: 06/05/2010
 *      Author: victor
 */

#ifndef NEWPLAYERTERMINALTEMPLATE_H_
#define NEWPLAYERTERMINALTEMPLATE_H_

#include "templates/SharedTangibleObjectTemplate.h"
#include "templates/tangible/NewPlayerMenuNode.h"

class NewPlayerTerminalTemplate : public SharedTangibleObjectTemplate {
	Reference<NewPlayerMenuNode*> rootNode;
	Vector<int> glowyBadgeIds;
	Vector<String> villageBranchUnlocks;

public:
	NewPlayerTerminalTemplate() : rootNode(nullptr) {
	}

	~NewPlayerTerminalTemplate() {
		if (rootNode != nullptr) {
			//delete rootNode;
			rootNode = nullptr;
		}
	...
	void readObject(LuaObject* templateData) {
		...
		if (luaItemList.getTableSize() % 2 != 0) {
			System::out << "[NewPlayerTerminalTemplate] Dimension mismatch in itemList. Item count must be a multiple of 2." << endl;
			luaItemList.pop();
			return;
		}

		rootNode = new NewPlayerMenuNode("root");
		rootNode->readLuaObject(luaItemList, true);

		luaItemList.pop();
    }

    inline const NewPlayerMenuNode* getItemList() const {
        return rootNode;
    }
...
#endif /* NEWPLAYERTERMINALTEMPLATE_H_ */
